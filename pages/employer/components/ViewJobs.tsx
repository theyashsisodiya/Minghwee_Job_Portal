...
import { MOCK_JOB_POSTINGS, JOB_CATEGORIES } from '../../../constants';
...
const JobCard: React.FC<{ job: JobPosting; onStatusToggle: (id: number) => void; navigate: ViewJobsProps['navigate'] }> = ({ job, onStatusToggle, navigate }) => {
...
const ViewJobs: React.FC<ViewJobsProps> = ({ navigate, country }) => {
    ...
    const toggleStatus = (id: number) => {
        setJobPostings(prevJobs =>
            prevJobs.map(job =>
                job.id === id
                    ? { ...job, status: job.status === JobStatus.Active ? JobStatus.Inactive : JobStatus.Active }
                    : job
            )
        );
    };
    ...
    const filteredAndSortedJobs = React.useMemo(() => {
        ...
        return filtered;
    }, [jobPostings, filterCategory, timeFilter]);
            ...
            {filteredAndSortedJobs.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredAndSortedJobs.map(job => (
                        <JobCard key={job.id} job={job} onStatusToggle={toggleStatus} navigate={navigate} />
                    ))}
                </div>
            ) : (
                <div className="text-center bg-white p-12 rounded-lg shadow-md border">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                        There are no jobs that match your current filters.
                    </p>
                </div>
            )}
        </div>
