...
import { MOCK_PRE_UPLOADED_DOCUMENTS } from '../../../constants';
import type { InterviewInvite, Notification } from '../../../types';
...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
    ...
    return <span className={`px-3 py-1 text-xs font-bold rounded-full ${styles[status] || styles['Active']}`}>{status}</span>;
}
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-lg">
                <h2 className="text-2xl font-bold mb-4 text-gray-800">Schedule Interview</h2>
                <p className="text-gray-600 mb-6">Select your pre-uploaded documents to submit for this application to confirm your interest.</p>
                ...
                <div className="mt-8 flex justify-end space-x-4">
                    ...
                    <button type="button" onClick={onSubmit} className="px-6 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700">Confirm & Schedule</button>
                </div>
            </div>
        </div>
    );
...
const InviteCard: React.FC<{ invite: InterviewInvite; applicationId: number; onAccept: (id: number) => void; }> = ({ invite, applicationId, onAccept }) => {
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200 hover:shadow-xl hover:border-blue-300 transition-all duration-300">
            <div className="flex items-start space-x-5">
             ...
             <div className="mt-6 pt-4 border-t flex justify-end items-center space-x-3">
                ...
                <button onClick={handleAcceptClick} className="px-5 py-2 text-sm bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 shadow-sm hover:shadow-lg transition-all">Accept & Schedule</button>
            </div>
        </div>
...
const InterviewInvites: React.FC<InterviewInvitesProps> = ({ addNotification }) => {
    const { getApplicationsByCandidate, employers, jobs, updateApplicationStatus, candidates } = useGlobalState();
    // Assuming current user is ID 1 for mock purposes if not authenticated properly, but generally handled by login
    // For this specific view component, we can grab the first candidate or handle logic differently.
    // Let's assume we are viewing for candidate with ID 1 (MOCK_CANDIDATE_PROFILE)
    const currentCandidateId = 1;
    const myApplications = getApplicationsByCandidate(currentCandidateId);
    ...
    const invites = useMemo(() => {
        return myApplications
            ...
            .map(app => {
                ...
                return {
                    id: app.id, // Using App ID for the invite ID mainly
                    ...
                    matchReason: 'Your profile matches the job requirements.',
                    interviewType: 'Live',
                    status: 'Active',
                    requiredDocuments: ['Passport', 'Bio-data']
                } as any; // Cast to any to fit the strict mock type for now
            });
    }, [myApplications, employers, jobs]);

    const handleAcceptInvite = (appId: number) => {
        updateApplicationStatus(appId, CandidateApplicationStatus.CandidateSelected); // Logic: Acceptance moves to Selected for scheduling/docs
        addNotification("Interview accepted! Please check 'Scheduled Interviews'.", 'success');
    };
            ...
            <div className="space-y-6">
                {invites.length > 0 ? (
                    ...
                    <div className="bg-white p-12 rounded-xl text-center border border-gray-200">
                        <p className="text-gray-500 font-medium">No pending interview invitations.</p>
                    </div>
                )}
            </div>
        </div>
